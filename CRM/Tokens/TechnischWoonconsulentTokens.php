...
class CRM_Tokens_TechnischWoonconsulentTokens {
  ...
  private function prefixToken(&$values, $cids, $token) {
    ...
    foreach($cids as $cid) {
      ...
      $this->setValue($values, $cid, $this->token_name.'.'.$token, $prefix);
    }
  }
  ...
  private function firstNameToken(&$values, $cids, $token) {
    ...
    foreach($cids as $cid) {
      ...
      $this->setValue($values, $cid, $this->token_name.'.'.$token, $name);
    }
  }
  ...
  protected function getSourceContactId($values, $cid) {
    if (isset($values[$cid])) {
      $value = $values[$cid];
    } else {
      $value = $values;
